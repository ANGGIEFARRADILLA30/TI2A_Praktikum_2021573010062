...
if($statusUpload == 0){
    ...
    if (mysqli_num_rows($select) > 0) {
        ...
        if(move_uploaded_file($_FILES['foto']['tmp_name'], $target_file)) {
            $query = mysqli_query($conn, "UPDATE daftar_menu SET foto='". $kode_rand . $_FILES['foto']['name'] . "', nama_menu='$nama_menu', keterangan='$keterangan', kategori='$kategori_menu', harga='$harga', stok='$stok' WHERE id ='$id'");
        if($query){
